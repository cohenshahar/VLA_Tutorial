"""
scene/demo_place_far.py  —  Phase 10, Task 10.4
═══════════════════════════════════════════════════════════════════════════════
Full sequence: reach → grasp → lift → transport → place (drop, 20 cm fall).

After lifting, the arm moves to above the target zone at "20 cm height":
  • EE descends until box bottom is 20 cm above target zone surface.
  • EM deactivates → box falls 20 cm and lands on the target zone.

Done when: target_touch_sensor > 0 within 2000 steps of EM deactivation.

Output
------
  outputs/demo_place_far_<YYYYMMDD>.mp4

Run:
  PYTHONPATH=VLATraining/sim python3 VLATraining/sim/scene/demo_place_far.py
"""
...
OUT_VIDEO = str(_SIM_DIR / "outputs" / f"demo_place_far_{_DATE}.mp4")
...
# "20 cm height" = box bottom 20 cm above target zone when released
# EE_Z = TARGET_Z + 0.20 + 2*BOX_HALF
PLACE_FAR_DROP = 0.20
PLACE_EE_Z = TARGET_Z + PLACE_FAR_DROP + 2 * BOX_HALF   # = 1.16 m

# Done criterion
CONTACT_TIMEOUT = 2000   # steps after EM deactivation
...
def main() -> None:
    """Run Task 10.4: reach → grasp → lift → transport → place far (20 cm drop)."""
    ...
    print(f"Place EE Z : {PLACE_EE_Z:.3f} m  (box bottom {PLACE_FAR_DROP*100:.0f} cm above target)")
    ...
    # ── 10. Descend to 20 cm release height ───────────────────────────────────
    print(f"[10] IK: descend to place Z = {PLACE_EE_Z:.3f} m "
          f"(box bottom {PLACE_FAR_DROP*100:.0f} cm above target) …")
    ok, err = _run_ik(model, data, dofas, qpas, ranges, site_id,
    ...
    # ── 11. EM deactivate → box drops 20 cm ──────────────────────────────────
    print("[11] EM deactivate — box drops 20 cm …")
    em_deactivate(model, data)

    # ── 12. Wait for contact (timeout = 2000 steps = 2 s) ────────────────────
